...
Public Class clsDataBase
    ...
    ''' ホスト名
    ''' </summary>
    Public Shared SrcHostName As String
    ''' <summary>
    ''' データベース名
    ''' </summary>
    Public Shared SrcDataBaseName As String

    ''' <summary>
    ''' ポート番号
    ''' </summary>
    Private Shared SrcPortNo As Integer
    ...
    ''' ホスト名
    ''' </summary>
    Public Shared BakHostName As String
    ''' <summary>
    ''' データベース名
    ''' </summary>
    Public Shared BakDataBaseName As String

    ''' <summary>
    ''' ポート番号
    ''' </summary>
    Private Shared BakPortNo As Integer
    ...
    Public Sub New()
        ...
        Dim IniFilePath As String =
           AppDomain.CurrentDomain.SetupInformation.ApplicationBase & "Backup.ini"

        If IO.File.Exists(IniFilePath) Then
            SrcHostName = GetIniString("SourceDataBase", "HostName", IniFilePath)
            SrcDataBaseName = GetIniString("SourceDataBase", "DataBaseName", IniFilePath)
            'HostName名に「:」がありその後の文字が整数であればそれをポート番号とする
            Dim dlmt As Integer = SrcHostName.IndexOf(":")
            If dlmt > 0 AndAlso IsNumeric(SrcHostName.Substring(dlmt + 1)) Then
                SrcPortNo = SrcHostName.Substring(dlmt + 1)
                SrcHostName = SrcHostName.Substring(0, dlmt)
            Else

                SrcPortNo = GetIniString("SourceDataBase", "port", IniFilePath)

            End If

            BakHostName = GetIniString("BackUpDataBase", "HostName", IniFilePath)
            BakDataBaseName = GetIniString("BackUpDataBase", "DataBaseName", IniFilePath)
            'HostName名に「:」がありその後の文字が整数であればそれをポート番号とする
            Dim dlm As Integer = BakHostName.IndexOf(":")
            If dlmt > 0 AndAlso IsNumeric(BakHostName.Substring(dlmt + 1)) Then
                BakPortNo = BakHostName.Substring(dlmt + 1)
                BakHostName = BakHostName.Substring(0, dlmt)
            Else

                BakPortNo = GetIniString("BackUpDataBase", "port", IniFilePath)
        ...
        Else
            MsgBox($"Backup.INIファイルが見つかりません。{vbCrLf}ファイルパス：{IniFilePath}",
    ...
    Private Function conMYSQLDB() As OdbcConnection
        ...
        Dim ConnectionString As String =
            $"DRIVER={DriverVersion};server={SrcHostName}; 
            database={SrcDataBaseName}; port={SrcPortNo}; 
        ...
        Catch ex As OdbcException
            ...
            If ex.Message.Contains("Unknown MySQL server host") Then
                ErrMsg = SrcHostName & ":ホスト名が見つかりません！  {ConnectionString}"
            End If
            'イベントログに書き込み
    ...
    Public Sub ExecuteSqlCmd(SQLCommand As String)
        ...
        Try
            ...
            Builder.Server = SrcHostName
            Builder.Port = SrcPortNo
            Builder.UserID = "toyo"
            Builder.Password = "yanagi"
            Builder.Database = SrcDataBaseName
...
Public Class clsInitParameter
    ...
    Public Sub New()
        ...
        Try
            ...
            Else
                _backUpFTPHostUserPass = $"ftp.toyoindustry.co.jp/flexbak/{SrcDataBaseName.Replace("flex", "")},flexbak,toyodaimon177"
            End If
